...
async def get_caterories(dialog_manager: DialogManager, **kwarg):
    category_list = [(i, get_num_from_cat(i, devices_with_categories_info)) for i in
                     list(devices_with_categories_info.keys())]
    return {"category_list": category_list}


async def get_equipment_from_cat(dialog_manager: DialogManager, **kwarg):
...
async def get_numbers_of_eq(dialog_manager: DialogManager, **kwarg):
    numbers_list = [(str(i), i) for i in range(101)]
    return {"numbers_list": numbers_list}


async def get_numbers_of_postamats(dialog_manager: DialogManager, **kwarg):
    numbers_list = [(str(i), i) for i in range(6)]
    return {"numbers_list": numbers_list}


async def get_rating_stars(dialog_manager: DialogManager, **kwarg):
    numbers_list = [(str(str(i)+'⭐'), i) for i in range(5,0,-1)]
    return {"numbers_list": numbers_list}


async def get_adding_status(dialog_manager: DialogManager, **kwarg):
...
async def get_delition_status(dialog_manager: DialogManager, **kwarg):
    basket_collection: Request_collection = dialog_manager.middleware_data.get("basket_collection")[
        int(dialog_manager.event.from_user.id)]
...
async def get_sent_status(dialog_manager: DialogManager, **kwarg):
    basket_collection: Request_collection = dialog_manager.middleware_data.get("basket_collection")[
        int(dialog_manager.event.from_user.id)]
...
async def get_found_equipment(dialog_manager: DialogManager, **kwarg):
...
async def get_changable_request_info(dialog_manager: DialogManager, **kwarg):
    basket_collection: Request_collection = dialog_manager.middleware_data.get("basket_collection")[
        int(dialog_manager.event.from_user.id)]
...
async def get_basket_requests_list(dialog_manager: DialogManager, **kwarg):
    basket_collection: Request_collection = dialog_manager.middleware_data.get("basket_collection")[
        int(dialog_manager.event.from_user.id)]
